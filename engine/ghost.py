...
def get_paths():
    ...
    if storage is None:
        # Fallback (ne devrait plus arriver après le premier lancement)
...
def recover_ghost(name: str, password: str = None, output_folder: str = "", recovery_key: str = None) -> bool:
    ...
    if recovery_key:
        # --- DEBUG TEMPORAIRE ---
        print("=== DEBUG RECOVERY KEY ===")
        print("Clé saisie :", recovery_key)
        print("has_recovery_key :", ghost.get("has_recovery_key"))
        print("recovery_hash enregistré :", ghost.get("recovery_hash"))
        print("hash de la clé saisie :", hashlib.sha256(recovery_key.encode()).hexdigest())
        # -------------------------
