...
class MainWindow:
    def __init__(self, root):
        ...
        # === CHOIX DU DOSSIER AU PREMIER LANCEMENT ===
        if get_storage_path() is None:
            dialog = SelectStorageDialog(self.root)
            self.root.wait_window(dialog)
            if dialog.result is None:
                # L'utilisateur a fermé sans choisir → on quitte
                self.root.destroy()
                return
    ...
    def recover_selected(self):
        ...
        password_dialog.geometry("420x280")
        password_dialog.resizable(False, False)
        password_dialog.transient(self.root)
        password_dialog.grab_set()
        ...
        ctk.CTkLabel(password_dialog, text="Recovery Key (emergency only):",
                     font=ctk.CTkFont(size=13)).pack(anchor="w", padx=40)
        recovery_entry = ctk.CTkEntry(password_dialog, width=320, height=34)
        recovery_entry.pack(pady=(0, 8))
        ...
        def do_recover():
            ...
            # ========== RECOVERY KEY (corrigé) ==========
            if recovery_key:
                # 1. On vérifie d'abord si la clé est valide (sans ouvrir de dossier)
                ...
                # 2. La clé est bonne → on demande le dossier
                password_dialog.destroy()
                output_folder = filedialog.askdirectory(title="Choose recovery folder")
                if not output_folder:
                    return

                # 3. On récupère vraiment les fichiers
